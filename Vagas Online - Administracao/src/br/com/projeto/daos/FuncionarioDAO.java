...
public class FuncionarioDAO implements GenericDAO<FuncionarioBean> {
	...
	private static final String INSERIR_FUNCIONARIO 		=	"INSERT INTO USUARIO(CPF, EMAIL, LOGIN, NOME, ID_PERFIL, RG, SEXO, ID_ESTACIONAMENTO_FUNC, STATUS) VALUES(?, ?, ?, ?, ?, ?, ?, ?, 'A')";
	...
	private static final String LISTAR_TODOS_FUNCIONARIOS_POR_ESTACIONAMENTO 		=	"SELECT ID, CPF, EMAIL, LOGIN, NOME, ID_PERFIL, RG, SEXO FROM USUARIO WHERE ID_ESTACIONAMENTO_FUNC = ?";
	...
	public boolean inserir(FuncionarioBean obj) {
		...
		try {
			...
			pstmt.setInt(8, obj.getIdEstacionamento());
			pstmt.executeUpdate();
		} catch (Exception e) {
